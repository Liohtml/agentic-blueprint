...
If the feature will clearly span more than one Chunk's worth of work (more than ~5 files, or more than a day), consider formalizing this output — the success criteria from the gate above — with the [SPEC template](../templates/SPEC.md.template); see [spec-driven-development.md](https://github.com/Liohtml/agentic-blueprint/blob/master/docs/spec-driven-development.md) for when it pays off and when an issue suffices.
